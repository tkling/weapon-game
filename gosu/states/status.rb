...
class Status < GameState
  ...
  def draw
    window.huge_font_draw(10, 10, 0, 'S T A T U S')

    window.large_font_draw( 35, 150,  0,  Color::YELLOW, 'party:')
    window.large_font_draw(150, 200, 50, *party.map do |pm|
      "#{pm.name} - level #{pm.level} - hp: #{pm.current_hp}/#{pm.max_hp} - xp: #{pm.xp}/#{Experience::LevelMap[pm.level+1]}"
    end)

    window.normal_font_draw(window.width/2-150, window.height/2+200, 0, '[space] return to caravan menu')
  end
end
